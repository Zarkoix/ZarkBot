...
function revokeRoles(member, tokens, roleGiver, message) {
  for (var index in tokens) {
    ...
    if (ROLES_LOWERCASE[token]) {
      (function () {
        var roleName = ROLES_LOWERCASE[token];
        console.log('attempting to revoke ' + roleName + ' from ' + member.displayName + ' on request of ' + message.author.displayName);
        member.removeRole(ROLES[roleName]).then(function () {
          if (member == roleGiver) {
            message.reply('you have been revoked the **' + roleName + '** role');
          } else {
            message.reply('revoked the **' + roleName + '** role from ' + member.displayName);
          }
        }).catch(function (error) {
          var response = 'could not revoke @' + member.displayName + ' the role' + roleName;
          console.error(response);
          console.error(error);
          message.reply(response);
        });
